...
select msi.*,
  BALANCE_TYPE, ITEM_SEGMENT, BEGINING_QUANTITY,BEGINING_AMOUNT,RECEIPTS_QUANTITY,RECEIPTS_AMOUNT,ISSUE_QUANTITY,ISSUE_AMOUNT,
  ENDING_QUANTITY,ENDING_AMOUNT, UNIT_PRICE,
  ITEM, SUBINVENTORY_CODE,  ON_HAND,
  wod.itemcd, otherlot1, orderdate,  expectqty1, shippedqty1
from (
    select   distinct   INVENTORY_ITEM_ID, SEGMENT1, SUMMARY_FLAG, ENABLED_FLAG, START_DATE_ACTIVE, END_DATE_ACTIVE, DESCRIPTION
     from MTL_SYSTEM_ITEMS_B a
    --WHERE         a.segment1 in ('VWSN10621301')
) MSI
left outer join (
    -- 목천 조직 수불마감 기준 단가 확인
    SELECT    ac.*, BALANCE_TYPE, ITEM_SEGMENT, BEGINING_QUANTITY,BEGINING_AMOUNT,RECEIPTS_QUANTITY,RECEIPTS_AMOUNT,ISSUE_QUANTITY,ISSUE_AMOUNT,ENDING_QUANTITY,ENDING_AMOUNT,
...
left outer join (
    -- WMS 주문 존재 여부 확인
    select  itemcd,  otherlot1, min(orderdate) orderdate,  sum(expectqty1) expectqty1, sum(shippedqty1) shippedqty1 from alexam.trso@alexam a
