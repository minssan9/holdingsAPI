SELECT VENDOR_NAME,
       ...
       SUM(CASE
               ...
               ELSE 0 END)                                                                      D120,
       SUM(
               CASE WHEN PERIOD_DATE > 120 THEN PO_PRICE * RP_QTY ELSE 0 END)                   D120_OVER,
       ...
       SUM(DISTINCT CASE
                        ...
                        ELSE PO_PRICE * A.PO_QTY END)                                           발주금액
FROM (
         ...
         FROM (
                  ...
                  FROM (
                           ...
                           INNER JOIN PO_VENDOR_SITES_ALL PVSA
                                      ON PHA.VENDOR_SITE_ID = PVSA.VENDOR_SITE_ID
                           INNER JOIN PO_LINES_ALL PLA ON PHA.PO_HEADER_ID = PLA.PO_HEADER_ID AND
                                                          PLLA.PO_HEADER_ID = PLA.PO_HEADER_ID AND
                                                          PLLA.PO_LINE_ID = PLA.PO_LINE_ID
                           LEFT OUTER JOIN PO_RELEASES_ALL PRA
                                           ON NVL(PLLA.PO_RELEASE_ID, -99999) = PRA.PO_RELEASE_ID
                           INNER JOIN MTL_SYSTEM_ITEMS_B MSIB
                                      ON PLA.ITEM_ID = MSIB.INVENTORY_ITEM_ID AND
                      ...
                      GROUP BY PO_HEADER_ID,
                               PO_LINE_ID
                  ) PLLAQTY ON PLLA.PO_HEADER_ID = PLLAQTY.PO_HEADER_ID AND
                               PLLA.PO_LINE_ID = PLLAQTY.PO_LINE_ID
                  WHERE PLLA.APPROVED_DATE BETWEEN :시작일자 AND :종료일자
